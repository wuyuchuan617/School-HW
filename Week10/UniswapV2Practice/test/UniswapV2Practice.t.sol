...
contract UniswapV2PracticeTest is Test {
    ...
    function test_maker_addLiquidityETH() public {
        ...
        UNISWAP_V2_ROUTER.addLiquidityETH{value: 100 ether}(
            address(testUSDC), 10000 * 10 ** testUSDC.decimals(), 0, 0, maker, block.timestamp + 100
        );
        vm.stopPrank();
    ...
    function test_taker_swapExactETHForTokens() public {
        ...
        UNISWAP_V2_ROUTER.swapExactETHForTokens{value: 1 ether}(0, path, taker, block.timestamp + 100);
        vm.stopPrank();
    ...
    function test_taker_swapExactTokensForETH() public {
        ...
        UNISWAP_V2_ROUTER.swapExactTokensForETH(100 * 10 ** testUSDC.decimals(), 0, path, taker, block.timestamp + 100);
    ...
    function test_maker_removeLiquidityETH() public {
        ...
        UNISWAP_V2_ROUTER.removeLiquidityETH(
            address(testUSDC), WETHTestUSDCPair.balanceOf(maker), 0, 0, maker, block.timestamp + 100
        );
        vm.stopPrank();

        // Checking
        IUniswapV2Pair wethUsdcPair1 = IUniswapV2Pair(UNISWAP_V2_FACTORY.getPair(address(WETH9), address(testUSDC)));
        (uint112 reserve0, uint112 reserve1,) = wethUsdcPair1.getReserves();
